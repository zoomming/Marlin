...
// Clean up unused ESP_WIFI pins
  ...
  #if !CONF_SERIAL_IS(ESP_WIFI_MODULE_COM)
 ...
 */
  ...
  #endif
  #if EXTJYERSUI && ENABLED(HAS_RC_CPU, AUTO_BED_LEVELING_BILINEAR, LIN_ADVANCE) && (PREHEAT_COUNT > 5)
    #error "With Linear Advance actived -> Max Preheat Label is 5 ! Modify User_config.h !"
  #endif
#endif

#if EXTJYERSUI && ENABLED(DWIN_CREALITY_LCD_JYERSUI)
  #if ENABLED(HAS_RC_CPU_AND_SHAPING, HAS_RC_CPU_AND_UBL) && EITHER(LIN_ADVANCE, HAS_JUNCTION_DEVIATION)
    #error "Not enought FLASH MEMORY With Junction Deviation feature or Linear Advance actived !"
    #error "Pas assez de MEMOIRE avec Junction Deviation ou Linear Advance activé(s) !"
  #endif
  #if ALL(HAS_RC_CPU_AND_UBL, HAS_RC_CPU_AND_MPC, HAS_BED_PROBE)
    #error "You cannot with a RCT6 CPU type, use MPC Tuning with UBL mode (not enough flash memory)"
    #error "Vous ne pouvez pas avec un Type CPU RCT6, utiliser MPC Tuning avec le mode UBL (pas assez de mémoire flash)."
  #endif
#endif
