...
public:
  ...
  bool      dryrun,
  ...
  #elif ABL_USES_GRID
    #if EXTJYERSUI
      TERN(PROBE_MANUALLY, int, const int) abl_points = GRID_MAX_POINTS;
    #else
      static constexpr int abl_points = GRID_MAX_POINTS;
    #endif
    //TERN(EXTJYERSUI, const, static constexpr) int abl_points = GRID_MAX_POINTS;
  #endif
    ...
    #else // Bilinear
      #if EXTJYERSUI
        xy_uint8_t grid_points = { HMI_datas.grid_max_points, HMI_datas.grid_max_points };
      #else
...
G29_TYPE GcodeSuite::G29() {
  ...
  if (!g29_in_progress) {
    ...
    probe.use_probing_tool();
      ...
      #if EXTJYERSUI && ENABLED(PROBE_MANUALLY)
        abl.abl_points = GRID_MAX_POINTS ;
        abl.grid_points.x = HMI_datas.grid_max_points ;
        abl.grid_points.y = HMI_datas.grid_max_points ;
      #endif
    #endif
  ...
  #else // !PROBE_MANUALLY
  {
    ...
    abl.measured_z = 0;
      ...
      for (PR_OUTER_VAR = 0; PR_OUTER_VAR < PR_OUTER_SIZE && !isnan(abl.measured_z); PR_OUTER_VAR++) {
        ...
        TERN_(EXTJYERSUI, if (temp_val.cancel_lev) break; );
        ...
        for (PR_INNER_VAR = inStart; PR_INNER_VAR != inStop; pt_index++, PR_INNER_VAR += inInc) {
          ...
          #else
            if (!temp_val.cancel_lev) { ExtJyersui.MeshSelectPoint(abl.meshCount.x, abl.meshCount.y, (pt_index== 1)); }
          #endif
          ...
          if (isnan(abl.measured_z)) {
            ...
            #if HAS_LIVEMESH
              if (!temp_val.cancel_lev) {
                char nstr_1[9];
                ui.status_printf(0, F(S_FMT " %i/%i -> z: " S_FMT), "Mesh Point", int(pt_index), int(abl.abl_points), dtostrf(z, 1, 3, nstr_1));
                ExtJyersui.MeshUpdate(abl.meshCount.x, abl.meshCount.y, z);
                }
            #endif
