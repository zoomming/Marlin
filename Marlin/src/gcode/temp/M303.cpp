...
void GcodeSuite::M303() {
  ...
  switch (hid) {
    ...
    */
    ...
    default:
      ...
      TERN_(DWIN_CREALITY_LCD_JYERSUI, CrealityDWINClass::PidTuning(pidresult_t::PID_BAD_HEATER_ID));
      return;
  }
  ...
  #if ENABLED(DWIN_CREALITY_LCD_JYERSUI) && EITHER(PIDTEMP, PIDTEMPBED)
    if (seenC) temp_val.PID_cycles = c;
    //if (seenS) { if (hid == H_BED) temp_val.PID_bed_temp = temp; else TERN_(PIDTEMP, temp_val.PID_e_temp = temp); }
    if (seenS) {
      switch (hid) {
        OPTCODE(PIDTEMP,    case 0 ... HOTENDS - 1: temp_val.PID_e_temp = temp; break)
        OPTCODE(PIDTEMPBED, case H_BED:             temp_val.PID_bed_temp = temp;    break)
        default: break;
      }
    }
  #endif
