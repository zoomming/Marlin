 ...
 *  T<200> or T<220>          Autotune the active extruder
 *                            with table 200 or 220.
 *                            Default T -> 200.
 * 
 *
...
void GcodeSuite::M306() {
  ...
  if (e >= (EXTRUDERS)) {
    ...
    return;
      ...
      if (parser.seenval('T')) {
        if (parser.value_celsius() == 220) { temp_val.MPCtargettemp = 220; temp_val.MPC_targetmode = 1; }
        else { temp_val.MPCtargettemp = 200; temp_val.MPC_targetmode = 0; }
    #else
...
void GcodeSuite::M306_report(const bool forReplay/*=true*/) {
  ...
  HOTEND_LOOP() {
    ...
    #if ENABLED(DWIN_CREALITY_LCD_JYERSUI)
      SERIAL_ECHOPAIR_F(" T", (float)temp_val.MPCtargettemp, 1);
    #endif
    SERIAL_EOL();
  }
}
