    ...
    switch (test_pattern) {
      ...
      case 1:
        LOOP_L_N(x, GRID_MAX_POINTS_X) {
          const uint8_t x2 = x + (x < (GRID_MAX_POINTS_Y) - 1 ? 1 : -1);                 // Create a diagonal line several Mesh cells thick that is raised
          z_values[x][x] += 9.999f;
  ...
  void unified_bed_leveling::probe_entire_mesh(const xy_pos_t &nearby, const bool do_ubl_mesh_map, const bool stow_probe, const bool do_furthest) {
    ...
    do {
      ...
      if (best.pos.x >= 0) {    // mesh point found and is reachable by probe
        ...
        #if HAS_LIVEMESH
          if (!temp_val.cancel_lev) { ExtJyersui.MeshSelectPoint(best.pos.x, best.pos.y, (point_num == 1)); }
        #endif
        ...
        z_values[best.pos.x][best.pos.y] = measured_z;
          ...
          if (!temp_val.cancel_lev) {
            char nstr_1[9];
            ui.status_printf(0, F(S_FMT " %i/%i -> z: " S_FMT), "Mesh Point", point_num, int(GRID_MAX_POINTS), dtostrf(measured_z, 1, 3, nstr_1));
            ExtJyersui.MeshUpdate(best.pos.x, best.pos.y, measured_z);
          }
        #endif
      }
...
bool unified_bed_leveling::smart_fill_one(const uint8_t x, const uint8_t y, const int8_t xdir, const int8_t ydir) {
  ...
  void unified_bed_leveling::tilt_mesh_based_on_probed_grid(const bool do_3_pt_leveling) {
    ...
    /*
    #if defined(G29J_MESH_TILT_MARGIN) && !EXTJYERSUI
       ...
       */
        ...
        */
        ...
        debug_pt(3, probe_pt[2], normal.z * gotz[2]);
          ...
          TERN(EXTJYERSUI, const, constexpr) xy_float_t safe_xy = { Z_SAFE_HOMING_X_POINT, Z_SAFE_HOMING_Y_POINT };
          d_from(); DEBUG_ECHOPGM("safe home with Z=");
