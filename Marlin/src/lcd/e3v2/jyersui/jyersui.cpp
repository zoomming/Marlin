...
#if HAS_GRAPHICAL_LEVELMANUAL && HAS_LEVELING && EXTJYERSUI
  uint16_t JYERSUI::last_backcolor = Color_Bg_Black;
#endif
...
//  value: float value
void JYERSUI::Draw_Float(uint8_t bShow, bool signedMode, uint8_t size, uint16_t color, uint16_t bColor, uint8_t iNum, uint8_t fNum, uint16_t x, uint16_t y, float value) {
  char nstr[10];
