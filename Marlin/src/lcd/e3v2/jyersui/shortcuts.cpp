...
#if BOTH(DWIN_CREALITY_LCD_JYERSUI, HAS_SHORTCUTS)
...
void ShortcutsClass::draw_moveZ() {
  ...
  DWIN_Draw_String(false, DWIN_FONT_STAT, Color_White, JYERSUI::backcolor, 107, 290, GET_TEXT_F(MSG_BUTTON_CANCEL));
  const uint8_t LM = 40;
  JYERSUI::cursor.x = LM;
  JYERSUI::cursor.y = 200;
  char nstr[6];
  sprintf_P(str, PSTR(" %s%s "), (rel_value >= 0)? "+" : "-" ,dtostrf(abs(rel_value), 4, 1, nstr));
...
void ShortcutsClass::onEncoderZ(EncoderState encoder_diffState) {
  switch (encoder_diffState) {
    // EncoderRate.encoderMoveValue
    case ENCODER_DIFF_CW:
      control_value = current_position.z + 1;    
