...
EncoderState Encoder_ReceiveAnalyze() {
  ...
  if (BUTTON_PRESSED(ENC)) {
    ...
    if (ELAPSED(now, next_click_update_ms)) {
      ...
      Encoder_tick();
        ...
        //LED_Action();
      #endif
      #if HAS_BACKLIGHT_TIMEOUT && ENABLED(DWIN_CREALITY_LCD_JYERSUI)
        if (!ui.backlight)  ui.refresh_backlight_timeout();
        else return ENCODER_DIFF_ENTER;
      #else
  ...
  if (ABS(temp_diff) >= ENCODER_PULSES_PER_STEP) {
    ...
    temp_diff = 0;
	  ...
	  if (temp_diffState != ENCODER_DIFF_NO)  ui.refresh_backlight_timeout();
  #endif
