...
#include "../core/debug_out.h"
  ...
  bool MarlinSettings::_load() {
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
      ...
      TERN_(DWIN_CREALITY_LCD_JYERSUI, LCD_MESSAGE(MSG_ERR_EEPROM_VERSION));
      ...
      if (eeprom_error) {
        ...
        TERN_(DWIN_CREALITY_LCD_JYERSUI, LCD_MESSAGE(MSG_ERR_EEPROM_CRC));
...
void MarlinSettings::reset() {
  ...
  void MarlinSettings::report(const bool forReplay) {
    ...
    //
    // EXTJYERSUI custom G-codes report
    //
    #if ENABLED(DWIN_CREALITY_LCD_JYERSUI) && HAS_LEVELING && HAS_LEVELING_HEAT && !HAS_NO_DWIN_C109
      CrealityDWIN.DWIN_C109_report();
    #endif
    #if EXTJYERSUI
      #if HAS_PHYSICAL_MENUS
        ExtJyersui.C100_report(forReplay);
        ExtJyersui.C101_report(forReplay);
        ExtJyersui.C102_report(forReplay);
      #endif
      #if ENABLED(NOZZLE_PARK_FEATURE)
        ExtJyersui.C125_report(forReplay);
      #endif
      #if HAS_MESH
        ExtJyersui.C29_report(forReplay);
        ExtJyersui.C852_report(forReplay);
      #endif
      #if HAS_BED_PROBE
        ExtJyersui.C851_report(forReplay);
      #endif
      ExtJyersui.C562_report(forReplay);
    #endif
  }
