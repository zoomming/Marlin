...
public:
    ...
    static constexpr xyz_pos_t offset = xyz_pos_t(LINEAR_AXIS_ARRAY_1(0)); // See #16767
  ...
  static bool stow(const bool no_return=false)   { return set_deployed(false, no_return); }
     ...
     */
    ...
    #else
        TERN(EXTJYERSUI, static, static constexpr) float _min_x(const xy_pos_t &probe_offset_xy=offset_xy) {
          #if EXTJYERSUI
            //const float &M_L = HMI_datas.mesh_min_x;
            return TERN(IS_KINEMATIC,
              (X_CENTER) - probe_radius(probe_offset_xy),
              _MAX((X_MIN_BED) + (HMI_datas.mesh_min_x), (X_MIN_POS) + probe_offset_xy.x)
            );
          #else
            return TERN(IS_KINEMATIC,
              (X_CENTER) - probe_radius(probe_offset_xy),
        ...
        TERN(EXTJYERSUI, static, static constexpr) float _max_x(const xy_pos_t &probe_offset_xy=offset_xy) {
          #if EXTJYERSUI
            //const float &M_R = HMI_datas.mesh_max_x;
            return TERN(IS_KINEMATIC,
              (X_CENTER) + probe_radius(probe_offset_xy),
              _MIN((HMI_datas.mesh_max_x), (X_MAX_POS) + probe_offset_xy.x)
            );
          #else
            return TERN(IS_KINEMATIC,
              (X_CENTER) + probe_radius(probe_offset_xy),
        ...
        TERN(EXTJYERSUI, static, static constexpr) float _min_y(const xy_pos_t &probe_offset_xy=offset_xy) {
          #if EXTJYERSUI
            //const float &M_F = HMI_datas.mesh_min_y;
            return TERN(IS_KINEMATIC,
              (Y_CENTER) - probe_radius(probe_offset_xy),
              _MAX((Y_MIN_BED) + (HMI_datas.mesh_min_y), (Y_MIN_POS) + probe_offset_xy.y)
            );
          #else
            return TERN(IS_KINEMATIC,
              (Y_CENTER) - probe_radius(probe_offset_xy),
        ...
        TERN(EXTJYERSUI, static, static constexpr) float _max_y(const xy_pos_t &probe_offset_xy=offset_xy) {
          #if EXTJYERSUI
            //const float &M_B = HMI_datas.mesh_max_y;
            return TERN(IS_KINEMATIC,
              (Y_CENTER) + probe_radius(probe_offset_xy),
              _MIN((HMI_datas.mesh_max_y), (Y_MAX_POS) + probe_offset_xy.y)
            );
          #else
            return TERN(IS_KINEMATIC,
              (Y_CENTER) + probe_radius(probe_offset_xy),
    ...
    public:
      ...
      static void get_three_points(T points[3]) {
          ...
          #elif ENABLED(AUTO_BED_LEVELING_UBL) && !HAS_RC_CPU_UBL_IS
