...
#if HAS_BED_PROBE && EXTJYERSUI // EXTJYERSUI !
 ...
 */
void do_blocking_move_to(LINEAR_AXIS_ARGS(const_float_t), const_feedRate_t fr_mm_s/*=0.0f*/) {
...
float get_move_distance(const xyze_pos_t &diff OPTARG(HAS_ROTATIONAL_AXES, bool &is_cartesian_move)) {
  if (!(LINEAR_AXIS_GANG(diff.x, || diff.y, /* skip z */, || diff.i, || diff.j, || diff.k /*, || diff.u, || diff.v, || diff.w*/)))
  ...
  inline bool line_to_destination_kinematic() {
    ...
    inline void segmented_line_to_destination(const_feedRate_t fr_mm_s, const float segment_size=LEVELED_SEGMENT_LENGTH) {
      ...
      */
      ...
      // delele -> SERIAL_ECHOLNPGM(" segment_mm=", cartesian_segment_mm);
  ...
  inline bool line_to_destination_cartesian() {
    const float scaled_fr_mm_s = MMS_SCALED(feedrate_mm_s);
      ...
      if (planner.leveling_active && planner.leveling_active_at_z(destination.z)) {
          ...
          #endif                                                        // all moves, including Z-only moves.
...
void prepare_line_to_destination() {
  ...
  bool homing_needed_error(linear_axis_bits_t axis_bits/*=linear_bits*/) {
    if ((axis_bits &= axes_should_home(axis_bits))) {
      ...
      char all_axes[] = STR_AXES_MAIN, need[LINEAR_AXES + 1];
      uint8_t n = 0;
      LOOP_LINEAR_AXES(i) if (TEST(axis_bits, i)) need[n++] = all_axes[i];
      need[n] = '\0';
      ...
      */
      ...
      SERIAL_ECHO_START();
      SERIAL_ECHOLN(msg);
      IF_DISABLED(DWIN_CREALITY_LCD_JYERSUI, sprintf_P(msg, GET_TEXT(MSG_HOME_FIRST), need));
      ui.set_status(msg);
      return true;
    }
    return false;
