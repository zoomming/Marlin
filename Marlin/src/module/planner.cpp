...
//void Planner::recalculate_trapezoids() {
  ...
  if (block) {
    ...
    // delete -> Reset next only to ensure its trapezoid is computed - The stepper is free to use
...
void Planner::check_axes_activity() {
  ...
  void Planner::autotemp_M104_M109() {
    _autotemp_update_from_hotend();
      ...
      const bool seen_U = parser.seen('U');
      if (seen_U) temp_val.autotemp_enable = parser.value_bool();
    #endif
    ...
    #if HAS_AUTOTEMP_MENU
      autotemp.factor = parser.seen('F') ? parser.value_float() : TERN(AUTOTEMP_PROPORTIONAL, AUTOTEMP_FACTOR_P, AUTOTEMP_FACTOR);
      if ((temp_val.autotemp_enable) && (autotemp.factor != 0)) autotemp.enabled = true;
      else { autotemp.enabled = false; temp_val.autotemp_enable = false; }
    #else
  ...
  TERN(EXTJYERSUI, ,constexpr) xy_pos_t level_fulcrum = {
...
bool Planner::_populate_block(block_t * const block,
  ...
  // Example 2: At 120°/s a 60° move involving only rotational axes takes 0.5s. So this will give 
