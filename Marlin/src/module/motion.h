...
#if HAS_BED_PROBE
  #if EXTJYERSUI
    extern feedRate_t z_probe_fast_mm_s; //EXTJYERSUI
  #else
...
inline int8_t pgm_read_any(const int8_t *p) { return TERN(__IMXRT1062__, *p, pgm_read_byte(p)); }
  ...
  #define XYZ_DEFS(T, NAME, OPT) \
    inline T NAME(const AxisEnum axis) { \
      const XYZval<T> Value = LINEAR_AXIS_ARRAY(X_##OPT, Y_##OPT, Z_##OPT, I_##OPT, J_##OPT, K_##OPT); \
      return Value[axis]; \
    }
#else
  #define XYZ_DEFS(T, NAME, OPT) \
...
typedef bits_t(LINEAR_AXES) linear_axis_bits_t;
...
typedef bits_t(LINEAR_AXES + EXTRUDERS) e_axis_bits_t;
