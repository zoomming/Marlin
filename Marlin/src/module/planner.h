...
typedef struct block_t {
  ...
  float nominal_speed,                   // The nominal speed for this block in (mm/sec)^2
...
class Planner {
  ...
  public:
     ...
     *  delete -> millimeters - the length of the movement, if known
     ...
     *  fr_mm_s     - (target) speed of the move
     *  del -> extruder    - target extruder
     *  del -> millimeters - the length of the movement, if known
  ...
  public:
     ...
     *  del -> extruder     - target extruder
     *  del -> millimeters  - the length of the movement, if known
     *  del -> inv_duration - the reciprocal if the duration of the movement, if known (kinematic only if feeedrate scaling is enabled)
     *  extruder     - optional target extruder (otherwise active_extruder)
     *  hints        - optional parameters to aid planner calculations
     */
