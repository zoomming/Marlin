 ...
 */
  ...
  void Temperature::PID_autotune(const celsius_t target, const heater_id_t heater_id, const int8_t ncycles, const bool set_result/*=false*/) {
    ...
    TERN_(DWIN_CREALITY_LCD_JYERSUI, CrealityDWINClass::PidTuning(isbed ? pidresult_t::PID_BED_START : pidresult_t::PID_EXTR_START));
    TERN_(EXTENSIBLE_UI, ExtUI::onPidTuning(ExtUI::result_t::PID_STARTED));
    TERN_(DWIN_PID_TUNE, DWIN_PidTuning(isbed ? PIDTEMPBED_START : PIDTEMP_START));
    

    if (target > GHV(CHAMBER_MAX_TARGET, BED_MAX_TARGET, temp_range[heater_id].maxtemp - (HOTEND_OVERSHOOT))) {
      TERN_(DWIN_CREALITY_LCD_JYERSUI, CrealityDWINClass::PidTuning(pidresult_t::PID_TEMP_TOO_HIGH));
    ...
    while (wait_for_heatup) { // Can be interrupted with M108
      ...
      if ((ms - _MIN(t1, t2)) > (MAX_CYCLE_TIME_PID_AUTOTUNE * 60L * 1000L)) {
        TERN_(DWIN_CREALITY_LCD_JYERSUI, CrealityDWINClass::PidTuning(pidresult_t::PID_TUNING_TIMEOUT));
      ...
      if (cycles > ncycles && cycles > 2) {
        ...
        TERN_(DWIN_CREALITY_LCD_JYERSUI, CrealityDWINClass::PidTuning(pidresult_t::PID_DONE));
  ...
  void Temperature::MPC_autotune(const uint8_t e) {
    auto housekeeping = [] (millis_t &ms, const uint8_t e, celsius_float_t &current_temp, millis_t &next_report_ms) {
      ...
      if (!wait_for_heatup) {
        ...
        TERN_(DWIN_CREALITY_LCD_JYERSUI, CrealityDWINClass::MPCTuning(pidresult_t::MPC_INTERRUPTED));
        return true;
      }
    ...
    TERN_(DWIN_CREALITY_LCD_JYERSUI, CrealityDWINClass::MPCTuning(pidresult_t::MPCTEMP_START));
    TERN(DWIN_CREALITY_LCD_JYERSUI, LCD_ALERTMESSAGE_F(STR_MPC_COOLING_TO_AMBIENT), LCD_MESSAGE(MSG_COOLING));
    ...
    hotend.modeled_ambient_temp = ambient_temp;
      ...
      switch (temp_val.MPC_targetmode) {
        case 0: SERIAL_ECHOLNPGM(STR_MPC_HEATING_PAST_200); LCD_ALERTMESSAGE(MSG_MPC_HEATING_PAST_200); break;
        case 1: SERIAL_ECHOLNPGM(STR_MPC_HEATING_PAST_220); LCD_ALERTMESSAGE(MSG_MPC_HEATING_PAST_220); break;
      }
      hotend.target = temp_val.MPCtargettemp;  // So M105 looks nice
    #else
      SERIAL_ECHOLNPGM(STR_MPC_HEATING_PAST_200);
      LCD_MESSAGE(MSG_HEATING);
      hotend.target = 200.0f;   // So M105 looks nice
    #endif
    //TERN(DWIN_CREALITY_LCD_JYERSUI, LCD_ALERTMESSAGE_F(STR_MPC_HEATING_PAST_200), LCD_MESSAGE(MSG_HEATING));
    //hotend.target = 200.0f;   // So M105 looks nice
    ...
    for (;;) { // Can be interrupted with M108
      ...
      if (ELAPSED(ms, next_test_ms)) {
        ...
        #if ENABLED(DWIN_CREALITY_LCD_JYERSUI)
          if (current_temp >= temp_val.MPCtargettemp) break;
        #else
    ...
    TERN(DWIN_CREALITY_LCD_JYERSUI, LCD_ALERTMESSAGE(MSG_MPC_MEASURING_AMBIENT), LCD_MESSAGE(MSG_MPC_MEASURING_AMBIENT));
    ...
    for (;;) { // Can be interrupted with M108
      ...
      if (!WITHIN(current_temp, t3 - 15.0f, hotend.target + 15.0f)) {
        ...
        TERN_(DWIN_CREALITY_LCD_JYERSUI, CrealityDWINClass::MPCTuning(pidresult_t::MPC_TEMP_ERROR));
        break;
      }
    }
    wait_for_heatup = false;
    ...
    TERN_(DWIN_CREALITY_LCD_JYERSUI, CrealityDWINClass::MPCTuning(pidresult_t::MPC_DONE));
